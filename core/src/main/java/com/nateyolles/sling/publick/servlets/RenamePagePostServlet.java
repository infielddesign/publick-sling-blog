 ...
 */
@SlingServlet(paths = PublickConstants.SERVLET_PATH_ADMIN + "/renamepage")
public class RenamePagePostServlet extends AdminServlet {

    /**
     * The logger.
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(RenamePagePostServlet.class);
    ...
    protected void doPost(SlingHttpServletRequest request, SlingHttpServletResponse response) throws ServletException, IOException {
        ...
        if (allowWrite) {
            ...
            final String renamePath = request.getParameter("renamePath");
            Resource existingNode = resolver.getResource(PublickConstants.CONTENT_PATH + "/" + renamePath);

            try {
                resolver.delete(existingNode);
                response.setStatus(SlingHttpServletResponse.SC_OK);
                sendResponse(writer, "OK", "Node " + renamePath + " successfully renamed.");
